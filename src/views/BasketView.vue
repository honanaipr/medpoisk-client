...
<template>
  <ListComponent
    :items="list_store.basketed"
    :item-component="BasketItem"
    @right="
      (item_id) => {
        list_store.unBasketById(item_id)
      }
    "
  >
    <template #empty_caption> В корзине ничего нет </template>
  ...
  <WriteOffFor
    v-model:doctor_id="doctor_id"
    v-model:room_id="room_id"
    :allow_apply="list_store.basketed.length && all_places_selected"
    @apply="apply"
    @cancel="cancel"
  />
</template>
